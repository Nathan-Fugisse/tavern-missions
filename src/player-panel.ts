...
import type { GuildBoard } from "./types";
...
function renderPlayerMissions(): void {
  ...
  activeMissions.forEach((m) => {
    ...
    if (btn) btn.addEventListener("click", () => handleClick(m.id));
  });

  updatePlayerStats();
}

async function handleClick(missionId: string): Promise<void> {
...
function updatePlayerStats(): void {
  ...
  currentBoard.missions.forEach((m) => {
    ...
    if (myClicks > 0) {
      ...
      if (m.status === "completed") missionsWon++;
    }
  });

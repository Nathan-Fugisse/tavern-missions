...
import { loadBoard, saveBoard, createMission, onBoardChange, formatTime, getProgressPercent, } from "./shared";
let currentBoard;
let timers = {};
export async function renderGMPanel(container) {
...
async function handleCreateMission() {
    ...
    const clicks = parseInt(document.getElementById("mission-clicks").value);
    const time = parseInt(document.getElementById("mission-time").value);
    if (!title) {
        alert("⚠️ Dê um título à missão!");
        return;
    }
    if (isNaN(clicks) || clicks < 1) {
        alert("⚠️ Meta mínima: 1 clique!");
        return;
    }
    if (isNaN(time) || time < 10) {
        alert("⚠️ Tempo mínimo: 10 segundos!");
        return;
    }
...
function updateHeader() {
    ...
    if (nameEl)
        nameEl.textContent = currentBoard.guildName;
}
...
window.deleteMission = async (id) => {
    if (!confirm("Tem certeza que quer remover essa missão?"))
        return;
    if (timers[id]) {
        clearInterval(timers[id]);
        delete timers[id];
